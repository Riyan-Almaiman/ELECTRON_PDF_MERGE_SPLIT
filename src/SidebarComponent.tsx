...
function SidebarComponent({ onUpdatePdf, onDeletePDF, pdfPages }: props) {
  ...
  const handleDeletePDF = (pdfToDelete: string, event: React.MouseEvent<SVGSVGElement>) => {
  ...
  >
    <div className="flex items-center justify-center" style={{ width: '5rem', height: '5rem' }}>
      {isLoading ? 
        <Spinner size={"xl"} color={"purple"} /> : 
        <div className="flex flex-col items-center justify-center">
          <HiOutlineUpload style={{ fontSize: "5rem", color: "white" }} />
...
<div className="w-full" style={separatorStyle}> </div>


<Sidebar.ItemGroup theme={{ base: "mt-4 space-y-2  pt-4 first:mt-0  first:pt-0 "
}}>
            {pdfPages.map((pdf, index) => (
              ...
              >
                <div className="flex items-center ">
                  <HiTrash
                  onClick={(event: React.MouseEvent<SVGSVGElement, MouseEvent>) => handleDeletePDF(pdf.pdfID, event)}
